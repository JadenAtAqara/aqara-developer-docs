---
title: "设备功能 API"
description: "本文档说明所有设备功能 API 的请求参数、响应参数、请求示例与响应示例。"
keywords: ["Aqara 文档", "Aqara Studio", "API 参考", "设备功能 API", "trait api"]
---
# 设备功能 API

## 分页查询设备信息

本接口用于分页查询设备信息列表。通过指定设备类型、排序字段、页码及每页大小等参数，可灵活筛选、排序并分页获取 Aqara Studio 中已接入设备的详细信息，包括设备 ID、名称、类型、添加时间、在线状态、所属空间等。
...
| data.data[].space | Object | 设备关联的空间信息。具体结构请参考 [Space 结构](#space-结构)。 |
...
| children | Array of Object | 子空间列表。具体结构请参考 [Space 结构](#space-结构)。若请求消息类型为 [GetDeviceInfoRequest](./trait-api.mdx#分页查询设备信息) 和 [GetAllDeviceInfoRequest](./trait-api.mdx#全量查询设备信息) 时，不会有此字段。 |
...
本接口用于全量查询设备信息列表。通过该接口，您可以一次性获取 Aqara Studio 内所有设备的详细信息，包括设备 ID、名称、设备类型、添加时间、在线状态、所属空间等。请求体支持根据设备类型、排序字段进行过滤。
...
| msgId | String | 是 | 由您定义的请求 ID。 |
| data | Object | 否 | 筛选配置。 |
| data.deviceTypesList | String[] | 否 | 筛选要查询的设备类型数组。若不填写，则默认查询所有类型设备。每个元素为具体的设备类型字符串，详见[设备类型说明](./device-type.mdx)。 |
| data.orderBy | String | 否 | 排序规则，支持以下字符串：<ul><li><code>createTime asc</code>：按照设备添加时间正序排列。</li><li><code>createTime desc</code>：按照设备添加时间倒序排列。</li></ul> |
...
| data[].deviceTypesList | Array of String | 设备类型。字符串含义请参考 [设备类型说明](./device-type.mdx)。 |
| data[].deviceName | String | 设备名称。 |
| data[].createTime | Number | 设备添加时间的 Unix 时间戳（毫秒）。 |
| data[].state | Boolean | 设备在线状态：<ul><li>false：离线。</li><li>true：在线。</li></ul> |
| data[].space | Object | 设备关联的空间信息。具体结构请参考 [Space 结构](#space-结构)。 |
...
| msgId | String | 是 | 由您定义的请求 ID。 |
| data | Object | 是 | 请求数据。 |
| data.deviceIds | Array of String | 否 | 将设备 ID 列表查询结果([分页](#分页查询设备信息) 或 [全量](#全量查询设备信息)) 传入此列表，作为设备的 spec 配置的查询目标。当此列表为空时，表示查询所有设备的 spec 配置。 |
...
| data[].endpoints[].functions[].traits[].parameter.type | String | 功能点值的类型，字符串含义请参考 [功能点值类型](#功能点值类型)。 |
