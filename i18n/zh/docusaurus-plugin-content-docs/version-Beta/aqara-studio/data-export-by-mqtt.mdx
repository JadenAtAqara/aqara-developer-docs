...
1. 在 Aqara Studio 左侧侧边栏进入 **开发者**，并开启 **MQTT转出** 服务。

    此时，界面会展示建立 MQTT 连接所需的所有信息（如 Aqara Studio 本机 IP、MQTT 端口、用户名、密码），以及请求和响应 Topic 的格式。
...
4. 在已创建的订阅中，将 Aqara Studio 提供的 **请求 Topic 格式**中的 ClientID 替换为 MQTTX 自动生成的实际 ClientID，并填写到 Topic 输入框。参照 设备功能 API 文档中各接口的请求参数说明，编写 JSON 格式的消息并发送，即可获得响应。

    下图以 查询设备 ID 列表 的请求为例：
