...
| data.deviceIds | Array of String | No | Pass the device ID list obtained from the [paginated query](#query-device-ids-by-pages) or the [complete query](#query-all-device-ids) of device IDs into this list as the target for querying the spec configuration of the devices. If this list is empty, the spec configuration for all devices will be queried. |
