---
title: "Device Function API"
description: "This document describes all device function APIs, including request parameters, response parameters, request examples, and response examples."
keywords: ["Aqara Docs", "Aqara Studio", "API Reference", "Device Function API"]
---
# Device Function API

## Query Device Information by Pages
...
| data.data[].space | Object | Space information associated with the device. For the detailed structure, refer to [Space Structure](#space-structure). |
...
| children          | Array of Object| List of child spaces. For detailed structure, refer to [Space Structure](#space-structure). If the request message type is [GetDeviceInfoRequest](./trait-api.mdx#query-device-information-by-pages) or [GetAllDeviceInfoRequest](./trait-api.mdx#query-all-device-information), this field will not be present. |
...
| data.deviceTypesList   | String[]   | No       | Array of device types to filter the query. If left empty, all device types will be queried by default. Each element represents a specific device type string. For details, see [Device Types](./device-type.mdx). |
| data.orderBy           | String     | No       | Sorting rule. Supported values:<ul><li><code>createTime asc</code>: Sort by creation time ascending.</li><li><code>createTime desc</code>: Sort by creation time descending.</li></ul> |
...
| data[].deviceTypesList | Array of String | Device types. For an explanation of the string values, see [Device Types](./device-type.mdx). |
| data[].deviceName | String | Device name. |
| data[].createTime | Number | The Unix timestamp (milliseconds) when the device was added. |
| data[].state | Boolean | Device online status:<ul><li>false: offline.</li><li>true: online.</li></ul> |
| data[].space | Object | Space information associated with the device. For the detailed structure, refer to [Space Structure](#space-structure). |
...
| data.deviceIds | Array of String | No | Pass the device ID list obtained from the [paginated query](#query-device-information-by-pages) or the [complete query](#query-all-device-information) of device IDs into this list as the target for querying the spec configuration of the devices. If this list is empty, the spec configuration for all devices will be queried. |
...
| data[].endpoints[].functions[].traits[].parameter.type | String | Type of trait value. For an explanation of the string values, see [Trait Value Types](#trait-value-types). |
...
After you [get the device's spec configuration](#query-device-spec-configuration), you can control the device by passing the relevant fields for the trait you want to operate and calling this API.
...
:::tip
A `code` value of `0` only means the API call was successful. It does not guarantee the device state has changed. Please [query the latest trait value](#query-the-latest-values-of-device-traits) to confirm if the control was effective.
::: 
...
| data.desc         | Boolean           | No       | Whether to sort query results in descending order by time: <ul><li>true: (default) descending</li><li>false: ascending</li></ul> |
| data.startTime    | Number            | Yes      | The query start timestamp, in milliseconds (UTC). |
| data.endTime      | Number            | Yes      | The query end timestamp, in milliseconds (UTC).<br/>**Note: The interval between start time and end time cannot exceed 7 days (`endTime - startTime ≤ 7 * 24 * 60 * 60 * 1000`)** |
| data.lang         | String            | Yes      | Language. <ul><li>en-US: English</li><li>zh-CN: Chinese</li></ul> |
